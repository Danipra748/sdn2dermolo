    ...
    <div class="glass rounded-3xl p-6 max-w-4xl">
        <form action="{{ $action }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Hero Color</label>
                    <input type="text" name="hero_color" value="{{ old('hero_color', $program->hero_color) }}"
                           class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-300"
                           placeholder="Contoh: from-blue-600 to-sky-600">
                    @error('hero_color')
                        <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                    @enderror
                </div>
