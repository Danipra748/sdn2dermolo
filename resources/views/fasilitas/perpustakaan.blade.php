        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
            @foreach ($data['stats'] as $stat)
                @php
                    ...
                    $colorClass = $colorMap[$stat['color']] ?? 'bg-emerald-100 text-emerald-600';
                @endphp
...
<section class="py-12 px-4 bg-white">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @foreach ($data['fasilitas_unggulan'] as $fasilitas)
                @php
                    $bgColors = [
                    ...
                    $bgClass = $bgColors[$fasilitas['bg']] ?? 'from-emerald-50 to-emerald-100';
                @endphp
                <div class="bg-gradient-to-br {{ $bgClass }} rounded-xl p-6 border border-slate-200">
                    <div class="text-4xl mb-3">{{ $fasilitas['icon'] }}</div>
                    <h3 class="text-xl font-bold text-slate-900 mb-2">{{ $fasilitas['title'] }}</h3>
                    <p class="text-slate-600">{{ $fasilitas['desc'] }}</p>
                </div>
            @endforeach
        </div>
    </div>
...
{{-- CTA Section --}}
<section class="py-16 px-4 bg-gradient-to-r from-emerald-600 to-teal-600 text-white">
    <div class="max-w-4xl mx-auto text-center">
        <h2 class="text-3xl font-bold mb-3">{{ $data['cta_title'] }}</h2>
        <p class="text-xl text-white/80 mb-8">{{ $data['cta_subtitle'] }}</p>
        <a href="{{ route('fasilitas.index') }}" 
           class="inline-flex items-center gap-2 px-8 py-4 bg-white text-emerald-600 font-bold rounded-full hover:bg-emerald-50 transition shadow-lg">
