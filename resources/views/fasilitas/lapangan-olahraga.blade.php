...
@section('title', 'Lapangan Olahraga - SD N 2 Dermolo')
...
<section class="py-16 px-4 bg-gradient-to-br from-indigo-50 to-purple-50">
    <div class="max-w-7xl mx-auto">
        <div class="mb-8">
            <a href="{{ route('fasilitas.index') }}" class="inline-flex items-center gap-2 text-indigo-600 hover:text-indigo-800 font-semibold transition">
                <- {{ $data['back_button_text'] ?? 'Kembali ke Fasilitas' }}
            </a>
        </div>

        <div class="bg-white rounded-2xl shadow-2xl p-8 mb-12 border-t-4 border-indigo-500">
        ...
        <div class="bg-white rounded-2xl shadow-2xl p-8 mb-12">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                @foreach(($data['stats'] ?? []) as $stat)
                    @php $c = $colors[$stat['color'] ?? 'indigo'] ?? $colors['indigo']; @endphp
        ...
        <div class="bg-white rounded-2xl shadow-2xl p-8 mb-12">
            ...
            <div class="grid md:grid-cols-3 gap-6">
                @foreach(($data['program'] ?? []) as $item)
                    <div class="p-6 border border-{{ $item['color'] ?? 'indigo' }}-100 rounded-xl bg-{{ $item['color'] ?? 'indigo' }}-50">
                        <h3 class="font-bold text-xl text-{{ $item['color'] ?? 'indigo' }}-900 mb-2">{{ $item['title'] ?? '' }}</h3>
...
<section class="py-16 px-4 bg-gradient-to-r from-indigo-600 to-purple-700">
    <div class="max-w-4xl mx-auto text-center">
        <h2 class="text-4xl font-bold text-white mb-4">{{ $data['cta_title'] ?? '' }}</h2>
        <p class="text-xl text-indigo-100 mb-8">{{ $data['cta_subtitle'] ?? '' }}</p>
        <a href="{{ route('home') }}#fasilitas" class="bg-white hover:bg-gray-100 text-indigo-700 font-bold py-4 px-10 rounded-full transition shadow-xl text-lg inline-block">
